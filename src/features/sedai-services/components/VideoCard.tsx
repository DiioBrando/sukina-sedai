...
export const VideoCard: React.FC<IAnime> = ({ list }) => {
    return (
        list.map((item) =>
        <div key={item.id} className={'flex flex-col flex-1 min-h-80 h-fit lg:max-h-[450px] min-w-[200px] max-w-[230px]  lg:h-full gap-1 pb-2'}>
            <Link className={'max-w-[230px]'} href={`/video/${item.id}`}>
                <Intro id={item.id} introName={item.posters.medium.url} className={'max-h-[300px] rounded-md'} width={230} height={300} ratingKinopoisk={0} ratingIMDb={0}/>
