...
export const VideoCard: React.FC<ISettingAnimeCard> = ({ animeObjectArray }) => {
    return (
        ...
        <div key={item.id} className={'flex flex-col flex-1 min-h-80 max-h-[450px] min-w-[200px] max-w-[300px] w-full h-full gap-1.5 pb-2'}>
            <Link href={`/${item.amineName}`}>
                <Intro id={item.id} introName={item.introName} className={'max-h-[300px]'} width={300} height={300} ratingKinopoisk={item.ratingKinopoisk} ratingIMDb={item.ratingIMDb}/>
